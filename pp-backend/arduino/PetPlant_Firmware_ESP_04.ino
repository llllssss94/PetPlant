...
const String USERCommand = String("AT+USER");
const String NAMECommand = String("AT+NAME");

const String CommnadLine = "(" + SHOWCommand + "," + SSIDCommand + ", " + PASSWDCommand + ", " + USERCommand + ", " + NAMECommand + ", " + ENDCommand + ")";

bool isConnect = false;
int count = 0;
String mSSID, mPASSWD, mUSER, mNAME;
...
void printSettingValue() {
  ...
  Serial.println("User ID : " + mUSER);
  Serial.println("Plant Name : " + mNAME);
}
...
void loop() {
  if (!isConnect) {
    if (Serial.available()) {
      ...
      } else if (str.equals(USERCommand)){
        Serial.println(str);
        Serial.println("Input New User ID!");
        while(1){
          if(Serial.available()){
            mUSER = Serial.readString();
            Serial.print("New User ID is '");
            Serial.print(mUSER);
            Serial.println("'");
            Serial.println("\nInput Command" + CommnadLine + "\n");
            break;
    ...
    sendData = mUSER+ parseData + mNAME + parseData + sendData;
